...
if s == "Disaster Classification":
    ...
    model = tf.keras.models.load_model("/Users/akshatjain/Desktop/CapstoneProject/DisasterFinal/model_DenseNet121.hdf5")
    ...
    if s == "Video":
        ...
        if f is None:
            ...
            # if frame is read correctly ret is True
                ...
                results[8] = prediction[0][8]+results[8]
                results[9] = prediction[0][9]+results[9]
            ...
            results[8] = results[8]/i
            results[9] = results[9]/i

          

            if results[0]==max(results) :
                st.write("P(no_disaster)--> ",round(results[0]*100),"%")
            if results[1]==max(results):
                st.write("P(Tsunami)    --> ",round(results[1]*100),"%")
            if results[2]==max(results):
                st.write("P(thunder and lightning)--> ",round(results[2]*100),"%")
            if results[3]==max(results):
                st.write("P(Drought)    -->  ",round(results[3]*100),"%")
            if results[4]==max(results):
                st.write("P(Cyclone)   -->   ",round(results[4]*100),"%")
            if results[5]==max(results):
                st.write("P(Earthquake)  -->   ",round(results[5]*100),"%")
            if results[6]==max(results):
                st.write("P(Flood)      -->  ",round(results[6]*100),"%")
            if results[7]==max(results):
                st.write("P(Wildfire)   -->  ",round(results[7]*100),"%")
            if results[8]==max(results):
                st.write("P(Hurricane)   -->  ",round(results[8]*100),"%")
            if results[9]==max(results):
                st.write("P(Typhoon)    -->   ",round(results[9]*100),"%")
        ...
        if file is None:
            ...
            if max(prediction[0]) == prediction[0][0] :
                st.write("P(no_disaster)--> ",round(prediction[0][0]*100),"%")
            if max(prediction[0]) == prediction[0][1] :
                st.write("P(Tsunami)    --> ",round(prediction[0][1]*100),"%")
            ...
            if max(prediction[0]) == prediction[0][3] :
                st.write("P(Drought)    -->  ",round(prediction[0][3]*100),"%")
            if max(prediction[0]) == prediction[0][4] :
                st.write("P(Cyclone)   -->   ",round(prediction[0][4]*100),"%")
            if max(prediction[0]) == prediction[0][5] :
                st.write("P(Earthquake)  -->   ",round(prediction[0][5]*100),"%")
            if max(prediction[0]) == prediction[0][6] :
                st.write("P(Flood)      -->  ",round(prediction[0][6]*100),"%")
            if max(prediction[0]) == prediction[0][7] :
                st.write("P(Wildfire)   -->  ",round(prediction[0][7]*100),"%")
            if max(prediction[0]) == prediction[0][8] :
                st.write("P(Hurricane)   -->  ",round(prediction[0][8]*100),"%")
            if max(prediction[0]) == prediction[0][9] :
                st.write("P(Typhoon)    -->   ",round(prediction[0][9]*100),"%")
...
# print(public_url)
    ...
    model = tf.keras.models.load_model("/Users/akshatjain/Desktop/CapstoneProject/DisasterFinal/model_VGG16 (2)-2.hdf5")
    st.title("Artificial Intelligence Based Disaster Management System")
    ...
    if file2 is None:
        ...
        st.write("Disaster Damage:",round(prediction[0][0]*100),"%")
